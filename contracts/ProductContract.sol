...
contract ProductContract{
    ...
    event VoteProductEvent(bytes32 productId, bytes32 voterId, bool agree, uint256 agreeCount, uint256 denyCount, ProductStatus afterStatus);
    //enum && structs
    enum ProductStatus {
        Approving,
        Approved,
    ...
    returns(bytes32 witnessDid, uint256 agreeCount, uint256 denyCount, ProductStatus afterStatus)
    {
        ...
        //event
        emit VoteProductEvent(productId, witnessDid, agree, agreeCount, denyCount, afterStatus);
    }

    function getProduct(bytes32 productId) external view returns(ProductInfo memory productInfo){
